...
bool PdBase::PdContext::init(const int numInChannels, const int numOutChannels, const int sampleRate, bool queued) {
    ...
    if(queued) {
        ...
        if(!bLibPdInited) {
            ...
            bLibPdInited = true;
            for(int i = 1; true; i++) {
                auto pure_data_startup_path_str = exec("defaults read org.puredata.pd path" + std::to_string(i));
                if (pure_data_startup_path_str.compare("") != 0) {
                    const char *pure_data_startup_path = pure_data_startup_path_str.c_str();
                    libpd_add_to_search_path(pure_data_startup_path);
                }
                else {
                    break;
                }
