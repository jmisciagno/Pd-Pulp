...
void PureDataAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    while (it.getNextEvent (message, samplePosition))
    {
        ...
        if (message.isAftertouch()) {
            pd->sendAftertouch(message.getChannel(), message.getAfterTouchValue());
        }
        if (message.isChannelPressure()) {
            pd->sendAftertouch(message.getChannel(), message.getChannelPressureValue());
        }
        if (message.isPitchWheel()) {
            pd->sendPitchBend(message.getChannel(), message.getPitchWheelValue() - 8192);
        }
